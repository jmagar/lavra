---
name: lavra-triage
description: Triage and prioritize beads one by one. Keep, modify, dismiss, or defer. Use for code review findings, security audits, performance analysis, or bulk categorization.
compatibility: Requires beads (bd) CLI, interactive user decisions
---

# Lavra Triage

Triage and categorize beads for prioritization. Present findings one by one and decide whether to keep, modify, dismiss, or defer them.

## When to Use

- After code review to prioritize all findings
- After security audits to categorize discovered issues
- After performance analysis to decide what to optimize
- When you have a large set of categorized findings that need tracking decisions
- To review and organize any collection of actionable items
...
3. If the bead doesn't exist:
   - Report: "Bead ID '#$ARGUMENTS' not found."
   - Stop execution
